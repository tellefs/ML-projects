...
study = "simple analysis" # "simple analysis", "epochs", "hidden layers", "Ridge grid search", "neurons"
NN_type = "self made" # "self made", "skl", "keras"
...
if(study == "simple analysis"):
	...
	if(NN_type == "self made"):
		...
		#plot_franke_test_train(z_predict,z_tilde, franke_data.X_test, franke_data.X_train, franke_data.scaler, franke_data.x_mesh, franke_data.y_mesh, franke_data.z_mesh)

	elif(NN_type == "skl"):

		regr = MLPRegressor(hidden_layer_sizes=(50,50,50),activation='logistic',solver='adam', alpha=lamb, batch_size=min_size, learning_rate_init=eta, max_iter=n_epochs).fit(franke_data.X_train, franke_data.z_train)
